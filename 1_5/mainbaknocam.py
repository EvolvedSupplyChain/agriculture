...
def main():
    ...
    while True:
        ...
        if mode==1:
            if start_capture==1:
                imageSense.flush_fifo()
                #time.sleep(1)
                imageSense.clear_fifo_flag()
                #time.sleep(1)
                imageSense.start_capture()
                #time.sleep(1)
                start_capture=0
            if imageSense.get_bit(ARDUCHIP_TRIG,CAP_DONE_MASK)!=0:
                read_fifo_burst()
                mode=0
                #time.sleep(1)
        ...
        #print(buffer)
        #print(imageBuff)
        #theFile = open("imageTemp.byte",'r')
        #tempImage = theFile.readlines()
        #print(tempImage)
        '''
